...
<script type="text/javascript">
	...
	$(function() { //자동으로 실행되는 코드
		...
		$("#btnList").click(function() {
			location.href = "${path}/qnaboard/list.do";
		});
		//수정 버튼
		$("#btnUpdate").click(
				function() {
					...
					$("#form1").append(str);
					document.form1.action = "${path}/qnaboard/update.do";
					document.form1.submit();
				});
		//삭제 버튼
		$("#btnDelete").click(function() {
			if (confirm("삭제하시겠습니까?")) {
				document.form1.action = "${path}/qnaboard/delete.do";
				document.form1.submit();
			}
		});
	...
	function listReply() {
		$.ajax({
			type : "get",
			url : "${path}/qnareply/list.do?bno=${dto.bno}",
			success : function(result) {
	...
	function listReply2() {
		$.ajax({
			...
			url : "${path}/qnareply/list_json.do?bno=${dto.bno}",
			success : function(result) {
			...
			<table border="1" width="100%">
				<tbody>
					...
					<tr class="main">
						<td colspan="4">
							<div class="detail">
								<img src="${path}/images/qnaboard/${dto.picture_url}" border="0">
								<br>
								<!-- iframe에 업로드 결과 출력 -->
